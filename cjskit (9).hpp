...
bytebuffer:
 ...
 `);
        ...
        static JSValue global_Promise_any(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
            ...
            std::thread t([=]() {
				...
				while (!isQuit && !jsmdPtr->isQuit) {
					...
					if (isFinished || error.size() >= GetLength(ctx, js_it)) break;
		...
		static uint64_t GetLength(JSContext* ctx, JSV object) {
			JSV js_length = GetProperty(ctx, object, "length");
			uint64_t length = 0;
		...
		static JSV GetProperty(JSContext* ctx, JSV targetObject, std::string prop) {
			if (!targetObject.isValid()) return JSV(JS_UNDEFINED);
		...
		static bool BindFunction(JSContext* ctx, JSV& targetFunction, JSV thisVal) {
			JSV bind = GetProperty(ctx, GetPrototype(ctx, targetFunction), "bind");
			if (!bind.isValid()) return false;
			JSV tempTargetFunction = CallFunction(ctx, bind, JS_UNDEFINED, {thisVal});
			if (tempTargetFunction.isValid()) {
		...
		static Promise NewPromise(JSContext* ctx) {
			...
			JSV thenFunc = NewFunction(ctx, "then", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				if (jsmdPtr->promiseList[id].state != PromiseState::PENDING) {
					if (!jsmdPtr->promiseList[id].callbacks.empty()) {
						if (jsmdPtr->promiseList[id].state == PromiseState::FULFILLED) {
							if (jsmdPtr->promiseList[id].callbacks.back().onFulfilled.isValid()) {
								JSV r = CallFunction(ctx, jsmdPtr->promiseList[id].callbacks.back().onFulfilled, thisVal, jsmdPtr->promiseList[id].result);
								if (!jsmdPtr->promiseList[id].callbacks.empty() && !JS_IsException(r.get(0))) {
									jsmdPtr->promiseList[id].callbacks.back().returnPromise.Resolve(ctx, r);
								}
								else if (!jsmdPtr->promiseList[id].callbacks.empty() && JS_IsException(r.get(0))) {
									jsmdPtr->promiseList[id].callbacks.back().returnPromise.Reject(ctx, r);
								}
							}
							else if (!jsmdPtr->promiseList[id].callbacks.empty()) {
								jsmdPtr->promiseList[id].callbacks.back().returnPromise.Resolve(ctx, jsmdPtr->promiseList[id].result[0]);
							}
						}
						else if (jsmdPtr->promiseList[id].state == PromiseState::REJECTED) {
							if (jsmdPtr->promiseList[id].callbacks.back().onRejected.isValid()) {
								JSV r = CallFunction(ctx, jsmdPtr->promiseList[id].callbacks.back().onRejected, thisVal, jsmdPtr->promiseList[id].error);
								if (!jsmdPtr->promiseList[id].callbacks.empty() && !JS_IsException(r.get(0))) {
									jsmdPtr->promiseList[id].callbacks.back().returnPromise.Resolve(ctx, r);
								}
								else if (!jsmdPtr->promiseList[id].callbacks.empty() && JS_IsException(r.get(0))) {
								...
								jsmdPtr->promiseList[id].callbacks.back().returnPromise.Reject(ctx, jsmdPtr->promiseList[id].error[0]);
							}
						}
					}
				}
			...
			JSV finallyFunc = NewFunction(ctx, "finally", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				if (jsmdPtr->promiseList[id].state != PromiseState::PENDING) {
					if (!jsmdPtr->promiseList[id].callbacks.empty()) {
						if (!jsmdPtr->promiseList[id].callbacks.back().isFinally) {
							if (!jsmdPtr->promiseList[id].callbacks.empty()) {
						...
						if (!jsmdPtr->promiseList[id].callbacks.back().onFinally.isValid()) {
							if (!jsmdPtr->promiseList[id].callbacks.empty()) {
								jsmdPtr->promiseList[id].state == PromiseState::FULFILLED
									? jsmdPtr->promiseList[id].callbacks.back().returnPromise.Resolve(ctx, jsmdPtr->promiseList[id].result[0])
									: jsmdPtr->promiseList[id].callbacks.back().returnPromise.Reject(ctx, jsmdPtr->promiseList[id].error[0]);
						...
						JSV fr = CallFunction(ctx, jsmdPtr->promiseList[id].callbacks.back().onFinally, thisVal, 0, nullptr);
						if (JS_IsException(fr.get(0))) {
							if (!jsmdPtr->promiseList[id].callbacks.empty()) {
								jsmdPtr->promiseList[id].callbacks.back().returnPromise.Reject(ctx, fr);
							}
							return newPromise.promise.get(1);
						}
						...
						JSV errFunc = NewFunction(ctx, "err", [](JSContext* ctx, JSValueConst thisVal, int, JSValueConst* argv)->JSValue {
							...
							if (!jsmdPtr->promiseList[id].callbacks.empty()) {
								jsmdPtr->promiseList[id].callbacks.back().returnPromise.Reject(ctx, JSV(ctx, argv[0]));
							}
							return JS_UNDEFINED;
							});
			...
			JSV resolve = NewFunction(ctx, "resolve", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				if (JS_IsSameValue(ctx, x.get(0), thisVal)) {
					...
					for (auto& cb : cbs) {
						if (cb.isFinally) {
							...
							JS_IsException(fr.get(0)) ? cb.returnPromise.Reject(ctx, fr) : cb.returnPromise.Reject(ctx, jsmdPtr->promiseList[id].error[0]);
						}
						else {
							cb.returnPromise.Reject(ctx, jsmdPtr->promiseList[id].error[0]);
						}
					}
					return JS_EXCEPTION;
				}
				...
				if (hasErr) {
					jsmdPtr->promiseList[id].isChanged = true;
					jsmdPtr->promiseList[id].state = PromiseState::REJECTED;
					jsmdPtr->promiseList[id].error.push_back(js_then);
					auto cbs = std::move(jsmdPtr->promiseList[id].callbacks);
					jsmdPtr->promiseList[id].callbacks.clear();
					for (auto& cb : cbs) {
						if (cb.isFinally) {
					...
					for (auto& cb : cbs) {
						if (cb.isFinally) {
							...
							if (JS_IsException(fr.get(0))) {
								...
								cb.returnPromise.Resolve(ctx, jsmdPtr->promiseList[id].result[0]);
							}
						}
						else if (cb.onFulfilled.isValid()) {
							JSV r = CallFunction(ctx, cb.onFulfilled, thisVal, jsmdPtr->promiseList[id].result);
				...
				if (JS_IsException(r.get(0)) && !jsmdPtr->promiseList[id].isChanged) {
					jsmdPtr->promiseList[id].isChanged = true;
					jsmdPtr->promiseList[id].state = PromiseState::REJECTED;
					jsmdPtr->promiseList[id].error.push_back(r);
					auto cbs = std::move(jsmdPtr->promiseList[id].callbacks);
					jsmdPtr->promiseList[id].callbacks.clear();
					for (auto& cb : cbs) {
						if (cb.isFinally) {
			...
			JSV reject = NewFunction(ctx, "reject", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				for (auto& cb : cbs) {
					if (cb.isFinally) {
						...
						JSV r = CallFunction(ctx, cb.onRejected, thisVal, jsmdPtr->promiseList[id].error);
						if (!JS_IsException(r.get(0))) {
							cb.returnPromise.Resolve(ctx, r);
						}
						else {
			...
			rp.callResolve = [=](JSContext* ctx, std::vector<JSV> args) -> JSV {
				return CallFunction(ctx, rp.resolve, promise, args);
				};
			rp.callReject = [=](JSContext* ctx, std::vector<JSV> args)-> JSV {
				return CallFunction(ctx, rp.reject, promise, args);
				};
			rp.Resolve = [=](JSContext* ctx, JSV arg)-> JSV {
				return CallFunction(ctx, rp.resolve, promise, { {arg.get(0) } });
				};
			rp.Reject = [=](JSContext* ctx, JSV arg)-> JSV {
				return CallFunction(ctx, rp.reject, promise, { {arg.get(0) } });
				};
			return rp;
		}
	...
	private:
		...
		static void runTask(JSContext* ctx) {
			...
			while (!jsmdPtr->taskList.empty() && !jsmdPtr->isQuit) {
				...
				JSV ret = CallFunction(ctx, task.task, task.thisVal, task.args, true);
