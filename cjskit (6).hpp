...
#undef PREFETCH
	...
	const char base85CodingMap[] = "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstu";
	...
	bool BaseXToBinary(BYTEBUFFER_PTR binaryPtr, uint64_t base, bool isUrl) {
		...
		size_t mapLen = 0;
		switch (base) {
		case 16: codingMap = base16CodingMap; mapLen = strlen(base16CodingMap); break;
		case 32: codingMap = base32CodingMap; mapLen = strlen(base32CodingMap); break;
		case 58: codingMap = base58CodingMap; mapLen = strlen(base58CodingMap); break;
		case 62: codingMap = base62CodingMap; mapLen = strlen(base62CodingMap); break;
		case 64: codingMap = isUrl ? base64UrlCodingMap : base64CodingMap; mapLen = strlen(isUrl ? base64UrlCodingMap : base64CodingMap); break;
		case 91: codingMap = isUrl ? base91UrlCodingMap : base91CodingMap; mapLen = strlen(isUrl ? base91UrlCodingMap : base91CodingMap); break;
		...
		if (base == 64 && padding > 2) return false;
		if (base == 32 && padding > 6) return false;
		if (base == 16 && padding > 0) return false;
	...
	bool BinaryToBaseX(BYTEBUFFER_PTR binaryPtr, uint64_t base, bool isUrl) {
		...
		if (base == 85) {
			...
			size_t pos = 0;
			// Adobe Ascii85 核心：4字节→5字符，严格适配UTF-8多字节
			while (pos < inputLen) {
				uint32_t acc = 0;
				size_t byteCount = 0;

				// 1. 读取最多4字节，大端序拼接（关键：UTF-8多字节需保持原始字节序）
				for (; byteCount < 4 && pos < inputLen; ++byteCount, ++pos) {
					acc = (acc << 8) | static_cast<uint8_t>(input[pos]);
				}

				// 2. 优化：4字节全0用z替代（避免生成5个!）
				if (byteCount == 4 && acc == 0) {
					result.push_back('z');
					continue;
				}

				// 3. 标准Base85编码：5次除以85取余（反向填充字符）
				char chars[5] = { 0 };
				int charIdx = 4;
				uint32_t temp = acc;

				// 关键修复：UTF-8场景下需用64位临时变量防溢出
				uint64_t bigTemp = static_cast<uint64_t>(temp);
				for (int i = 0; i < 5; i++) {
					chars[charIdx--] = base85CodingMap[bigTemp % 85];
					bigTemp = bigTemp / 85;
				}

				// 4. 仅保留有效字符数（byteCount+1），适配UTF-8不足4字节的末尾分组
		...
		size_t mapLen = 0;
		switch (base) {
		case 16: codingMap = "0123456789ABCDEF"; mapLen = 16; break;
		case 32: codingMap = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567"; mapLen = 32; break;
		case 58: codingMap = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"; mapLen = 58; break;
		case 62: codingMap = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"; mapLen = 62; break;
		case 64: codingMap = isUrl ? "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_" : "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"; mapLen = 64; break;
		case 91: codingMap = isUrl ? "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!#$%&()*+,-./:;<=>?@[]^_`{|}~" : "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!#$%&()*+,-./:;<=>?@[]^_`{|}~"; mapLen = 91; break;
		default: return false;
		}
		if (mapLen != base) return false;
		...
		switch (base) {
		case 16: paddingCount = result.size() % 2 == 1 ? 1 : 0; if (paddingCount > 0) result.insert(result.begin(), '0'); break;
		...
		for (size_t i = 0; i < paddingCount; ++i) {
			if (base == 32 || base == 64) result.push_back('=');
		}
	...
	public:
		JavaScriptMethod(JavaScript* InInstance, JSRuntime* InjsRuntime, JSContext* InjsContext) {
            ...
            AppendMethod(ctx, global, "include", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues)->JSValue {
				...
				std::unordered_map<std::string, std::string> chosedModules = {};
				...
				if (isAll || chosedModules.count("cjs:system")) {
					...
					SetAttribute(ctx, system, "onLine", NewBool(ctx, IsDeviceOnline()));
			...
			if (!isModuleMode) CallFunction(ctx, GetProperty(ctx, global, "include"), global, {{NewString(ctx, "all")}});
...
window:
    ...
    sleep(milliseconds: number):void: 阻塞等待指定的时间(毫秒)，由于受到系统时钟精度的影响，实际等待实际可能存在10毫秒以内的波动；(milliseconds: number)指定的时间，仅可为正整数或0；无返回值；数据不正确抛出类型错误。
 ...
 `);
		...
		static JSValue network_http_open(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			xhr->onreadystatechange = [=]() {
                ...
                if (callback.isValid() && JS_IsFunction(ctx, callback.get(0))) CallFunction(ctx, callback, returnValue, {});
				};
			...
			auto onload = [=](JSV callback) {
				...
				if (callback.isValid() && JS_IsFunction(ctx, callback.get(0))) CallFunction(ctx, callback, returnValue, { {callbackValue} });
				};
            xhr->onload = [=](...) {
		...
		static JSValue global_Blob(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			AppendMethod(ctx, blob, "slice", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues)->JSValue {
				...
				if (end - start < data.size()) {
					data.erase(data.begin() + (end - start), data.end());
				}
		...
		static JSValue global_setTimeout(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			std::thread t([=](){
				...
				while (GetTickCount64() < targetTime && !isQuit && jsmdPtr->timeoutList[id].shared == nullptr) {
					ULONGLONG remaining = targetTime - GetTickCount64();
					AdvSleep(static_cast<double>(remaining > 1 ? 1 : remaining));
				}

				if (!isQuit && jsmdPtr->timeoutList[id].shared == nullptr) {
					CallFunction(ctx, js_callback, NewGlobalObject(ctx), {});
                }
		...
		static JSValue global_await(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			PromiseData& promiseData = jsmdPtr->promiseList[id];

			while (!isQuit) {
				if (promiseData.state != PromiseState::PENDING) {
					break;
				}
				AdvSleep(1.0);
			}

			size_t size = promiseData.result.size();
			if (promiseData.state == PromiseState::FULFIFILLED) {
				return (size <= 1)?(size==0)?JS_UNDEFINED:promiseData.result[0].get(1):NewArray(ctx, promiseData.result).get(1);
			}
			else {
				JSV error = (size <= 1) ? (size == 0) ? JS_UNDEFINED : promiseData.result[0].get(1) : NewArray(ctx, promiseData.error).get(1);
				JS_ThrowPlainError(ctx, ("(in promise) " + ToString(ctx, error)).c_str());
				return JS_EXCEPTION;
			}
			...
			std::thread t([=]() {
				...
				if (format != "jwk") {
					...
					if (jwk[L"kty"].get<std::string>() == "oct") {
						...
						if (crv != "P-192" && crv != "P-256" && crv != "P-384" && crv != "P-521" && crv != "secp192k1" && crv != "secp224r1" && crv != "secp224k1" && crv != "secp256k1") {
				...
				if (pkd.isValid) {
					...
					if (a_name == "HMAC") {
						...
						JSV js_a_length = {};
						uint64_t a_length = 0;
						if (!ReadObjectProperty(ctx, js_algorithm, "length", js_a_length) || !ReadJSValueAsUint64(ctx, js_a_length, a_length)) {
		...
		static JSValue crypto_subtle_generateKey(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			if (a_name == "HMAC") {
				...
				if (a_hash_name != "SHA-1" && a_hash_name != "SHA-224" && a_hash_name != "SHA-256" && a_hash_name != "SHA-384" && a_hash_name != "SHA-512" && a_hash_name != "SHA-3-224" && a_hash_name != "SHA-3-256" && a_hash_name != "SHA-3-384" && a_hash_name != "SHA-3-512" && a_hash_name != "SHA-512/224" && a_hash_name != "SHA-512/256") {
					promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.generateKey] RSA hash name must be SHA-1/SHA-224/SHA-256/SHA-384/SHA-512/SHA-3-224/SHA-3-256/SHA-3-384/SHA-3-512/SHA-512/224/SHA-512/256"));
					return promise.promise.get(1);
				}
				...
				if (a_namedCurve != "P-192" && a_namedCurve != "P-256" && a_namedCurve != "P-384" && a_namedCurve != "P-521" && a_namedCurve != "secp192k1" && a_namedCurve != "secp224r1" && a_namedCurve != "secp224k1" && a_namedCurve != "secp256k1") {
					promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.generateKey] EC name curve must be P-192/P-256/P-384/P-521/secp192k1/secp224r1/secp224k1/secp256k1"));
					return promise.promise.get(1);
				}
				...
				if (a_name == "ECDSA") {
					...
					if (ReadObjectProperty(ctx, uAlgorithm, "hash", js_a_hash)) {
						...
						if (a_hash_name != "SHA-1" && a_hash_name != "SHA-224" && a_hash_name != "SHA-256" && a_hash_name != "SHA-384" && a_hash_name != "SHA-512" && a_hash_name != "SHA-3-224" && a_hash_name != "SHA-3-256" && a_hash_name != "SHA-3-384" && a_hash_name != "SHA-3-512" && a_hash_name != "SHA-512/224" && a_hash_name != "SHA-512/256") {
							promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.generateKey] ECDSA hash name must be SHA-1/SHA-224/SHA-256/SHA-384/SHA-512/SHA-3-224/SHA-3-256/SHA-3-384/SHA-3-512/SHA-512/224/SHA-512/256"));
							return promise.promise.get(1);
						}
					}
				}
				...
				if (JS_IsObject(val) || JS_IsArray(val)) {
					...
					if (tag_str == "Promise") {
						...
						if (!JS_IsException(state_val) && !JS_IsUndefined(state_val) && !JS_IsNull(state_val)) {
							std::string state_str = js_val_to_string(state_val);
							if (!state_str.empty()) promise_str = L"Promise { <" + stringToWstring(state_str) + L"> }";
						}
		...
		static JSV NewObject(JSContext* ctx, OBJECT& object) {
			...
			for (const auto& [key, value] : object) {
				...
				if (value.isString()) {
					...
					attributeValue = NewObject(ctx, nextObject);
				}
				else {
		...
		static Promise NewPromise(JSContext* ctx) {
			...
			AppendMethod(ctx, promise, "then", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				if (argumentCount < 1) {
					JS_ThrowTypeError(ctx, "[Promise] Only 1 or 2 arguments are supported: (resolver, rejecter?)");
					return JS_EXCEPTION;
				}
				JSV js_resolver = JSV(ctx, argumentValues[0]).cget(1).cset(1);
				JSV js_rejecter = (argumentCount >= 2) ? JSV(ctx, argumentValues[1]).cget(1).cset(1) : NewFunction(ctx, "", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue { return JS_UNDEFINED; });
				...
				promiseData.thenResolve = js_resolver;
				promiseData.thenReject = js_rejecter;

				if (promiseData.state != PromiseState::PENDING && !promiseData.isCalled) {
					bool isffd = (promiseData.state == PromiseState::FULFIFILLED);
					CallFunction(ctx, (isffd) ? promiseData.thenResolve : promiseData.thenReject, thisVal, (isffd) ? promiseData.result : promiseData.error);
					promiseData.isCalled = true;
					promiseData.error.clear();
				}

				promiseData.thenCallback = JSV(ctx, thisVal).cget(1).cset(1);

				Promise newPromise = NewPromise(ctx);
				promiseData.returnPromise = newPromise;
			...
			AppendMethod(ctx, promise, "catch", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				if (argumentCount < 1) {
					JS_ThrowTypeError(ctx, "[Promise] Only 1 argument is supported: (callback)");
					return JS_EXCEPTION;
				}
				...
				promiseData.catchCallback = JSV(ctx, thisVal).cget(1).cset(1);
				promiseData.catchReject = js_callback;

				if (promiseData.state == PromiseState::REJECTED && !promiseData.isCalled) {
					CallFunction(ctx, promiseData.catchReject, thisVal, promiseData.error);
					promiseData.isCalled = true;
					promiseData.error.clear();
				}
			...
			AppendMethod(ctx, promise, "finally", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				promiseData.finallyCallback = JSV(ctx, thisVal).cget(1).cset(1);
				promiseData.finallyFinish = js_callback;

				if (promiseData.state != PromiseState::PENDING && !promiseData.isCalled) {
					CallFunction(ctx, promiseData.finallyFinish, thisVal, 0, nullptr);
					promiseData.isCalled = true;
				}
			...
			JSV resolve = NewFunction(ctx, "resolve", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				if (!promiseData.isValid || promiseData.isCalled || promiseData.isChanged) return JS_UNDEFINED;

				std::vector<JSV> tempArgs = {};
				for (int i = 0; i < argumentCount; i++) {
					tempArgs.push_back(JSV(ctx, argumentValues[i]).cget(1).cset(1));
				}

				promiseData.result = tempArgs;
				promiseData.state = PromiseState::FULFIFILLED;
				promiseData.isChanged = true;

				JSV returnValue;
				if (promiseData.finallyFinish.isValid()) {
					returnValue = CallFunction(ctx, promiseData.finallyFinish, thisVal, {});
				}
				else {
					returnValue = CallFunction(ctx, promiseData.thenResolve, thisVal, tempArgs);
				}
				promiseData.isCalled = true;

				if (GetSymbolName(ctx, returnValue) == "Promise") {
					JSV js_new_id = GetProperty(ctx, returnValue, { {"internal"}, {"id"} });
					ULL new_id = 0;
					ReadJSValueAsUint64(ctx, js_new_id, new_id);
					if (!jsmdPtr->promiseList.count(new_id)) {
						if (promiseData.returnPromise.Reject) promiseData.returnPromise.Reject(ctx, NewInternalError(ctx, "[Promise] This promise instance is invalid"));
						return JS_UNDEFINED;
					}

					PromiseData& new_promiseData = jsmdPtr->promiseList[new_id];
					new_promiseData.resolve = promiseData.returnPromise.resolve;
					new_promiseData.reject = promiseData.returnPromise.reject;

					if (new_promiseData.isChanged == true) {
						if (new_promiseData.state == PromiseState::FULFIFILLED) {
							if (promiseData.returnPromise.callResolve) promiseData.returnPromise.callResolve(ctx, new_promiseData.result);
						}
						else if (new_promiseData.state == PromiseState::REJECTED) {
							if (promiseData.returnPromise.callReject) promiseData.returnPromise.callReject(ctx, new_promiseData.error);
						}
					}

				}
				else {
					if (!JS_IsException(returnValue.get(0))) {
						if (promiseData.returnPromise.Resolve) promiseData.returnPromise.Resolve(ctx, returnValue);
					}
					else {
						if (promiseData.returnPromise.Reject) promiseData.returnPromise.Reject(ctx, returnValue);
					}
			...
			JSV reject = NewFunction(ctx, "reject", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
				...
				promiseData.error.insert(promiseData.error.end(), tempArgs.begin(), tempArgs.end());
				promiseData.result = tempArgs;
				promiseData.state = PromiseState::REJECTED;
				promiseData.isChanged = true;

				JSV returnValue;
				if (promiseData.catchReject.isValid()) {
					returnValue = CallFunction(ctx, promiseData.catchReject, thisVal, promiseData.error);
				}
				else {
					returnValue = CallFunction(ctx, promiseData.thenReject, thisVal, tempArgs);
				}
				if (promiseData.catchReject.isValid()) promiseData.error.clear();
				promiseData.isCalled = true;
			...
			promiseData.isValid = true;
			promiseData.state = PromiseState::PENDING;
			jsmdPtr->promiseList[id] = promiseData;
			...
			returnPromise.callResolve = [=](JSContext* ctx, std::vector<JSV> args) -> JSV {
				return CallFunction(ctx, returnPromise.resolve, promise, args);
				};
			returnPromise.callReject = [=](JSContext* ctx, std::vector<JSV> args)-> JSV {
				return CallFunction(ctx, returnPromise.reject, promise, args);
				};
			returnPromise.Resolve = [=](JSContext* ctx, JSV arg)-> JSV {
				JSValue jsArg = arg.get(0);
				return CallFunction(ctx, returnPromise.resolve, promise, 1, &jsArg);
				};
			returnPromise.Reject = [=](JSContext* ctx, JSV arg)-> JSV {
				JSValue jsArg = arg.get(0);
				return CallFunction(ctx, returnPromise.reject, promise, 1, &jsArg);
				};
	...
	private:
		...
		static void update(JSContext* ctx) {
            ...
            const static int updateInterval = 100;
            static ULONGLONG lastTime = GetTickCount64();
			if (lastTime + updateInterval > GetTickCount64()) return;
