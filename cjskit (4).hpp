...
#undef PREFETCH
    ...
    #undef ERROR
    std::unordered_map<std::wstring, BYTEBUFFER> URLDataList = {};
	...
	class URLInstanceClass {
	public:
		URLInstanceClass(const std::wstring url, unordered_map_lock<URL_T_PTR, bool>* controller = nullptr) {
			ProcessMemory(controller);
			ProcessURL(url);
			UpdateInfo();
		};
		URLInstanceClass(const std::wstring path, const std::wstring baseUrl, unordered_map_lock<URL_T_PTR, bool>* controller = nullptr) {
			ProcessMemory(controller);
			ProcessURL(baseUrl + path);
			UpdateInfo();
		};
		~URLInstanceClass() {
			if (URLController != nullptr && URLController->count(this)) {
				URLController->erase(this);  //已手动释放
			}
		}
	...
	private:
		...
		void ProcessMemory(unordered_map_lock<URL_T_PTR, bool>* controller) {
			if (controller != nullptr && isMemoryOnHeap(this)) {

				if (controller->count(this)) goto MemoryProcessEnd;
		...
		URLINFO info = {};
		unordered_map_lock<URL_T_PTR, bool>* URLController = nullptr;
	};
	...
	public:
		...
		~URLStaticClass() {
			for (auto& [urlInstance, unused] : URLController) {
				if (urlInstance) {
					delete urlInstance;
				}
			}
		...
		URL_T_PTR operator()(const std::wstring url) {
			return new URLInstanceClass(url, &URLController);
		}
		URL_T_PTR operator()(const std::wstring path, const std::wstring baseUrl) {
			return new URLInstanceClass(path, baseUrl, &URLController);
		}

		static std::wstring createObjectURL(const BYTEBUFFER_PTR data) {
	...
	private:
		unordered_map_lock<URL_T_PTR, bool> URLController = {};
	};
			...
			try {
				...
				if (urlInfo.protocol == L"blob:") {
					if (bodyPtr == nullptr && formDataPtr == nullptr && method == L"GET" && URLDataList.count(urlInfo.href)) {
						BYTEBUFFER_PTR bp = &URLDataList[urlInfo.href];
						responseText = GetTextFromBYTEBUFFER(bp);
						responseType = GetMIMETypeFromBYTEBUFFER(bp);
						response = *bp;
						onFinish();
						return true;
					}
 ...
 `);
		...
		static JSValue global_URL_createObjectURL(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			if (GetSymbolName(ctx, js_object) == "Blob") {
				...
				std::string url = wstringToString(URL.createObjectURL(&data));
		...
		static JSValue global_URL(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			URL_T urlp = URL(stringToWstring(url));
		...
		static JSValue global_Blob(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			SetAttribute(ctx, internal, "data", NewArrayBuffer(ctx, blobData));
		...
		static JSValue global_await(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			if (finallyState == JS_PROMISE_FULFILLED) {
				...
				if (error == nullptr) {
					errorString = ToString(ctx, JSV(ctx, &result));
