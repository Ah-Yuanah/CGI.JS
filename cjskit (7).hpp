...
#undef ERROR
    ...
    public:
        ...
        bool hung() {
            if (threadInst->isHung()) return false;
            DWORD suspendCount = SuspendThread(getHandle());
        ...
        bool resume() {
            if (!threadInst->isHung()) return false;
            DWORD resumeCount = ResumeThread(getHandle());
    ...
    struct Promise {
        ...
        std::function<void(JSContext* ctx, std::vector<JSV> args)> callResolve = nullptr;
        std::function<void(JSContext* ctx, std::vector<JSV> args)> callReject = nullptr;
    ...
    struct PromiseData {
        ...
        std::vector<JSV> result = {}; // fulfilled 结果（代码中存储/读取）
        std::vector<JSV> error = {};  // rejected 错误（代码中存储/读取）
        std::vector<PromiseCallback> callbacks; // 回调队列（代码中核心使用）
    };
...
bytebuffer:
 ...
 `);
        ...
        static JSValue global_await(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
            ...
            if (promiseData.state == PromiseState::FULFILLED) {
        ...
        static Promise NewPromise(JSContext* ctx) {
            ...
            AppendMethod(ctx, promise, "then", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
                ...
                PromiseData& pd = jsmdPtr->promiseList[id];

                Promise newPromise = NewPromise(ctx);
                pd.callbacks.push_back({ js_resolver, js_rejecter, JSV(), newPromise, false });

                std::thread t([=]() {

                    if (pd.state != PromiseState::PENDING) {
                        auto& cb = pd.callbacks.back();
                        if (pd.state == PromiseState::FULFILLED) {
                            if (cb.onFulfilled.isValid()) {
                                JSV r = CallFunction(ctx, cb.onFulfilled, thisVal, pd.result, true);
                                ...
                                cb.returnPromise.Resolve(ctx, pd.result[0]);
                            }
                        }
                        else if (pd.state == PromiseState::REJECTED) {
                            if (cb.onRejected.isValid()) {
                                JSV r = CallFunction(ctx, cb.onRejected, thisVal, pd.error, true);
                                if (!JS_IsException(r.get(0))) {
                                    cb.returnPromise.Resolve(ctx, r);
                                }
                                else {
                                ...
                                cb.returnPromise.Reject(ctx, pd.error[0]);
                            }
                        }
                    }
            ...
            AppendMethod(ctx, promise, "finally", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
                ...
                PromiseData& pd = jsmdPtr->promiseList[id];
                Promise newPromise = NewPromise(ctx);
                pd.callbacks.push_back({ JSV(), JSV(), js_callback, newPromise, true });
                ...
                if (pd.state != PromiseState::PENDING) {
                    ...
                    JSV errFunc = NewFunction(ctx, "err", [](JSContext* ctx, JSValueConst thisVal, int, JSValueConst* argv)->JSValue {
                        JSV js_id = GetProperty(ctx, thisVal, { {"internal"}, {"id"} });
                        ULL id = 0;
                        ReadJSValueAsUint64(ctx, js_id, id);
                        JSMData* jsmdPtr = nullptr;
                        ...
                        PromiseData& pd = jsmdPtr->promiseList[id];
                        auto& cb = pd.callbacks.back();
                        cb.returnPromise.Reject(ctx, JSV(ctx, argv[0]));
                        return JS_UNDEFINED;
                        });
            ...
            JSV resolve = NewFunction(ctx, "resolve", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
                ...
                PromiseData& pd = jsmdPtr->promiseList[id];
                if (pd.isChanged) return JS_UNDEFINED;
                ...
                std::thread t([=, &pd]() {
                    ...
                    if (JS_IsSameValue(ctx, x.get(0), js_this.get(0))) {
                        pd.isChanged = true;
                        pd.state = PromiseState::REJECTED;
                        pd.error = { JSV(ctx, JS_ThrowTypeError(ctx, "[Promise] Chaining cycle detected for promise")) };
                        auto cbs = std::move(pd.callbacks);
                        pd.callbacks.clear();
                        for (auto& cb : cbs) {
                            if (cb.isFinally) {
                                JSV fr = CallFunction(ctx, cb.onFinally, thisVal, 0, nullptr, true);
                                JS_IsException(fr.get(0)) ? cb.returnPromise.Reject(ctx, fr) : cb.returnPromise.Reject(ctx, pd.error[0]);
                            }
                            else {
                                cb.returnPromise.Reject(ctx, pd.error[0]);
                            }
                    ...
                    if (!JS_IsFunction(ctx, js_then.get(0))) {
                        ...
                        for (auto& cb : cbs) {
                            if (cb.isFinally) {
                                ...
                                JSV r = CallFunction(ctx, cb.onFulfilled, thisVal, pd.result, true);
                                !JS_IsException(r.get(0)) ? cb.returnPromise.Resolve(ctx, r) : cb.returnPromise.Reject(ctx, r);
                            }
                            else {
                                cb.returnPromise.Resolve(ctx, pd.result[0]);
                            }
                    ...
                    auto res = [](JSContext* ctx, JSValueConst thisVal, int argc, JSValueConst* argv)->JSValue {
                        JSV js_id = GetProperty(ctx, thisVal, { {"internal"}, {"id"} });
                        ULL id = 0;
                        ReadJSValueAsUint64(ctx, js_id, id);
                        JSMData* jsmdPtr = nullptr;
                        ...
                        PromiseData& pd = jsmdPtr->promiseList[id];
                        if (pd.isChanged) return JS_UNDEFINED;
                        return CallFunction(ctx, pd.resolve, thisVal, argc, argv, true).get(1);
                        };
                    auto rej = [](JSContext* ctx, JSValueConst thisVal, int argc, JSValueConst* argv)->JSValue {
                        JSV js_id = GetProperty(ctx, thisVal, { {"internal"}, {"id"} });
                        ULL id = 0;
                        ReadJSValueAsUint64(ctx, js_id, id);
                        JSMData* jsmdPtr = nullptr;
                        ...
                        PromiseData& pd = jsmdPtr->promiseList[id];
                        if (pd.isChanged) return JS_UNDEFINED;
                        return CallFunction(ctx, pd.reject, thisVal, argc, argv, true).get(1);
                        };

                    JSV r = CallFunction(ctx, js_then, x, {
                        NewFunction(ctx, "resolve", res, 0),
                        NewFunction(ctx, "reject", rej, 0),
                        }, true);
                    if (JS_IsException(r.get(0)) && !pd.isChanged) {
                        pd.isChanged = true;
                        pd.state = PromiseState::REJECTED;
                        pd.error.push_back(r);
                        auto cbs = std::move(pd.callbacks);
                        pd.callbacks.clear();
                        for (auto& cb : cbs) {
                            if (cb.isFinally) {
            ...
            JSV reject = NewFunction(ctx, "reject", [](JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) ->JSValue {
                ...
                std::thread t([=, tempArgs = std::move(tempArgs), &pd]() {
            ...
            rp.resolve = resolve;
            rp.reject = reject;
            rp.callResolve = [=](JSContext* ctx, std::vector<JSV> args) {
                CallFunction(ctx, rp.resolve, promise, args, true, false);
                };
            rp.callReject = [=](JSContext* ctx, std::vector<JSV> args) {
                CallFunction(ctx, rp.reject, promise, args, true, false);
                };
            rp.Resolve = [=](JSContext* ctx, JSV arg) {
                CallFunction(ctx, rp.resolve, promise, { {arg.get(0) } }, true, false);
                };
            rp.Reject = [=](JSContext* ctx, JSV arg) {
                CallFunction(ctx, rp.reject, promise, { {arg.get(0) } }, true, false);
                };
            return rp;
        }
        ...
        static JSV CallFunction(JSContext* ctx, JSV func, JSV thisVal, std::vector<JSV> args, bool isAsync = false, bool isWait = true) {
            ...
            JSV result = JSV();
            if (isAsync) {
                ULL id = addTask(ctx, result, thisVal, args);
    ...
    private:
        ...
        static ULL addTask(JSContext* ctx, JSV task, JSV thisVal, std::vector<JSV> args) {
            JSMData* jsmdPtr = nullptr;
            if (!GetData(ctx, &jsmdPtr) || jsmdPtr == nullptr) return 0;
        ...
        static void runTask(JSContext* ctx) {
            ...
            while (!jsmdPtr->taskList.empty()) {
                ...
                JSV ret = CallFunction(ctx, task.task, task.thisVal, task.args, false);
