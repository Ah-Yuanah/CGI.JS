...
#include "./thirdParty/cryptopp/include/md5.h"
...
namespace cjs {
	...
	static std::unordered_map<std::wstring, std::wstring> TextLightColorValue = {
		...
		// ========== 新增：通用基础颜色（差异化高，适配WT终端全色系） ==========
		{L"Gray", L"#CCCCCC"},        // 浅灰（基础通用灰）
		{L"DarkGray", L"#888888"},    // 深灰（弱化版深灰）
		{L"LightGray", L"#EEEEEE"},   // 极浅灰（接近白）
		{L"SlateGray", L"#708090"},   // 石板灰（中性冷灰，区别于DarkGray）
		{L"Silver", L"#C0C0C0"},      // 银灰（经典中性灰，区别于Gray）
		{L"Green", L"#44FF44"},       // 亮绿（复用成功色）
		{L"ForestGreen", L"#228B22"}, // 森林绿（深绿系，区别于DarkGreen）
		{L"LimeGreen", L"#32CD32"},   // 石灰绿（亮绿系，区别于Green）
		{L"Red", L"#FF4444"},         // 亮红（复用错误色）
		{L"Crimson", L"#DC143C"},     // 绯红（深红系，区别于DarkRed）
		{L"Tomato", L"#FF6347"},      // 番茄红（暖红系，区别于Red）
		{L"Yellow", L"#FFFF44"},      // 亮黄（复用警告色）
		{L"Gold", L"#FFD700"},        // 金色（暖黄系，区别于Yellow）
		{L"Khaki", L"#F0E68C"},       // 卡其黄（浅黄系，区别于Yellow）
		{L"Blue", L"#4444FF"},        // 亮蓝（复用字符串色）
		{L"RoyalBlue", L"#4169E1"},   // 皇家蓝（深蓝系，区别于DarkBlue）
		{L"SkyBlue", L"#87CEEB"},     // 天蓝色（浅蓝系，区别于Blue）
		{L"Cyan", L"#44FFFF"},        // 亮青（复用信息色）
		{L"Teal", L"#008080"},        // 水鸭青（深青系，区别于Cyan）
		{L"Magenta", L"#FF44FF"},     // 亮洋红（复用关键字色）
		{L"Fuchsia", L"#FF00FF"},     // 品红（纯洋红，区别于Magenta）
		{L"Orchid", L"#DA70D6"},      // 兰花紫（浅洋红系，区别于Magenta）
		{L"Orange", L"#FF8844"},      // 亮橙（复用数值色）
		{L"Chocolate", L"#D2691E"},   // 巧克力橙（深橙系，区别于Orange）
		{L"Peach", L"#FFE5B4"},       // 桃色（浅橙系，区别于Orange）
		{L"Purple", L"#CC88FF"},      // 亮紫（复用空值色）
		{L"Violet", L"#EE82EE"},      // 紫罗兰（浅紫系，区别于Purple）
		{L"Indigo", L"#4B0082"},      // 靛蓝（深紫系，区别于Purple）
		{L"DarkGreen", L"#22AA22"},   // 深绿（控制台适配版）
		{L"DarkRed", L"#AA2222"},     // 深红（控制台适配版）
		{L"DarkBlue", L"#2222AA"},    // 深蓝（控制台适配版）

		// ========== 原有：基础提示类（微调1种，提升WT终端区分度） ==========
		{L"Error", L"#FF4444"},    // 错误-亮红（保留）
		{L"Warn", L"#FFFF44"},     // 警告-亮黄（保留）
		{L"Success", L"#44FF44"},  // 成功-亮绿（保留）
		{L"Info", L"#87CEEB"},     // 信息-天蓝色（替换原亮青，WT终端更柔和）

		// ========== 原有：JS核心语法/关键字（微调2种，避免重复） ==========
		{L"Keyword", L"#FF44FF"},  // 关键字-亮洋红（保留）
		{L"Operator", L"#FFFFFF"}, // 操作符-纯白（保留）
		{L"String", L"#4444FF"},   // 字符串-亮蓝（保留）
		{L"Number", L"#FF8844"},   // 数值-亮橙（保留）
		{L"Boolean", L"#C0C0C0"},  // 布尔-银灰（替换原浅灰，区别于Default）
		{L"NullUndefined", L"#EE82EE"},// 空值-紫罗兰（替换原亮紫，区别于Purple）
		{L"Punctuator", L"#AAAAAA"},// 标点-浅灰（保留）
		{L"Comment", L"#888888"},  // 注释-深灰（替换原浅灰，WT终端对比更清晰）

		// ========== 原有：JS内置/自定义对象/函数/类（无重复，保留） ==========
		{L"BuiltInObject", L"#4444FF"}, // 内置对象-亮蓝
		{L"BuiltInFunction", L"#44FFFF"},// 内置函数-亮青
		{L"Class", L"#FF44FF"},    // 类/构造-亮洋红
		{L"Function", L"#44FF44"}, // 函数-亮绿
		{L"Method", L"#44FF44"},   // 方法-亮绿
		{L"Property", L"#CCCCCC"}, // 属性-浅灰
		{L"Variable", L"#FFFFFF"}, // 变量-纯白
		{L"Constant", L"#FF8844"}, // 常量-亮橙

		// ========== 原有：JS高级特性/特殊类型（微调2种，丰富度提升） ==========
		{L"Module", L"#4169E1"},   // 模块-皇家蓝（替换原亮蓝，区别于String）
		{L"Promise", L"#4B0082"},  // 异步-靛蓝（替换原亮紫，区别于NullUndefined）
		{L"RegExp", L"#FF44FF"},   // 正则-亮洋红
		{L"Symbol", L"#44FFFF"},   // 符号-亮青
		{L"Type", L"#44FF44"},     // 类型-亮绿

		// ========== 原有：通用辅助色（微调1种，适配WT终端） ==========
		{L"Debug", L"#32CD32"},    // 调试-石灰绿（替换原亮绿，区别于Success）
		{L"Highlight", L"#FFD700"},// 高亮-金色（替换原亮黄，WT终端更醒目）
		{L"Default", L"#CCCCCC"},  // 默认-浅灰
		{L"Black", L"#000000"},    // 黑色
		{L"White", L"#FFFFFF"}     // 白色
	};
	...
	public:
		...
		// 查找（原方法保留，补全count、contains(C++20)、lower_bound/upper_bound/equal_range）
		...
		// 下界（返回第一个>=key的迭代器，本实现为插入顺序，故返回find或end）
		...
		// 上界（返回第一个>key的迭代器，本实现为插入顺序，故返回find+1或end）
		...
		// 等范围（返回[lower_bound, upper_bound)，本实现为单元素或空）
		...
		// 插入（原方法保留，补全所有标准插入重载：右值、emplace、范围、初始化列表）
		std::pair<iterator, bool> insert(const value_type& pair) {
			...
			m_index_map[pair.first] = m_data.size();
		...
		std::pair<iterator, bool> insert(value_type&& pair) {
			...
			size_type idx = m_data.size();
			m_index_map[pair.first] = idx;
			m_data.push_back(std::move(pair));
			return { m_data.begin() + idx, true };
		}
		// 带提示的插入（本实现为插入顺序，提示无效，仅作兼容）
		iterator insert(const_iterator hint, const value_type& pair) {
			(void)hint; // 忽略提示，因本map保持插入顺序
		...
		// 原位构造（emplace）：直接在容器内构造键值对，避免拷贝/移动
	...
	struct OBJECTStruct {
		using DataVariant = std::variant<
			...
			unsigned int, unsigned long, unsigned long long, nullptr_t, OBJECT
		>;
		DataVariant data;
		...
		bool isWString() const { return std::holds_alternative<std::wstring>(data); }
		...
		OBJECT* operator->() { return &std::get<OBJECT>(this->data); }
		const OBJECT* operator->() const { return &std::get<OBJECT>(this->data); }
		template <typename T>
		...
		static OBJECT parse(std::wstring jsonText, JSONPARSEREVIVER reviver = nullptr) {
			...
			std::function<OBJECTStruct(std::wstring)> parseValue = [&](std::wstring key) -> OBJECTStruct {
				...
				if (ptr[pos] == L'{') {
					...
					throw std::invalid_argument("Arrays not supported");
				}
				else {
			...
			while (pos < len && ptr[pos] != L'\0' && std::iswspace(ptr[pos])) pos++;
			if (pos >= len || ptr[pos] != L'{') {
				throw std::invalid_argument("Root must be object");
			}
	...
	struct JSMData {
		...
		std::unordered_map<ULL, OBJECT> attributeList = {};
	};
	...
	static std::unordered_map<std::string, std::unordered_map<std::string, std::string>> allowedKeyUsagesList = {
		...
		// 新增：ChaCha20-Poly1305（Web Crypto标准用途）
		{"ChaCha20-Poly1305", {
			{"encrypt", ""},
			{"decrypt", ""},
			{"wrapKey", ""},
	...
	PKDATA GetPKData(BYTEBUFFER_PTR keyBinaryPtr) {
		...
		byteQueue.Put(keyBinaryPtr->data(), keyBinaryPtr->size());
		byteQueue.MessageEnd();

		try {
			CryptoPP::InvertibleRSAFunction rsaPrivKey;
			rsaPrivKey.Load(byteQueue);

			pkData.isPrivate = true;
			pkData.name = "RSASSA-PKCS1-v1_5";
		...
		try {
			...
			if (byteQueue.MaxRetrievable() >= 32) {
				byteQueue.Get(sk, 32);
			...
			try {
				...
				if (byteQueue.MaxRetrievable() >= 32) {
					byteQueue.Get(pk, 32);
					CryptoPP::ed25519Verifier verifier(pk);
					pkData.isPrivate = false;
					pkData.name = "Ed25519";
					pkData.isValid = true;
					return pkData;
		...
		try {
			...
			if (byteQueue.MaxRetrievable() >= 32) {
				...
				x25519Key.GeneratePublicKey(rngTmp, sk, pk);
				pkData.isPrivate = true;
				pkData.name = "X25519";
				pkData.isValid = true;
				return pkData;
	...
	KEYDATA GetKeyData(BYTEBUFFER_PTR keyBinaryPtr) {
		KEYDATA keyData;
		if (keyBinaryPtr == nullptr || keyBinaryPtr->empty()) {
			return keyData;
		}

		size_t keyLen = keyBinaryPtr->size();
		const unsigned char* keyDataPtr = keyBinaryPtr->data();

		try {
			CryptoPP::AES::Encryption aesEnc;
			aesEnc.SetKey(keyDataPtr, keyLen);

			keyData.length = static_cast<uint64_t>(keyLen * 8);
			if (keyData.length == 128) {
				keyData.name = "AES-GCM";
			}
			else if (keyData.length == 192) {
				keyData.name = "AES-CBC";
			}
			else if (keyData.length == 256) {
				keyData.name = "AES-CTR";
			}
			else {
				keyData.name = "AES-KW";
			}
		...
		if (keyLen == 32) {
			try {
				...
				chacha.SetKey(keyDataPtr, 32);
				keyData.name = "ChaCha20-Poly1305";
				keyData.length = 256;
				keyData.isValid = true;
				return keyData;
			}
		...
		auto validateHmac = [&](const std::string& hashAlg, const unsigned char* k, size_t l) -> bool {
			try {
				if (hashAlg == "SHA-256") { CryptoPP::HMAC<CryptoPP::SHA256> h(k, l); return true; }
				if (hashAlg == "SHA-1") { CryptoPP::HMAC<CryptoPP::SHA1> h(k, l); return true; }
				if (hashAlg == "SHA-512") { CryptoPP::HMAC<CryptoPP::SHA512> h(k, l); return true; }
				if (hashAlg == "MD5") { CryptoPP::HMAC<CryptoPP::Weak::MD5> h(k, l); return true; }
				if (hashAlg == "SHA3-256") { CryptoPP::HMAC<CryptoPP::SHA3_256> h(k, l); return true; }
				return false;
			}
			catch (...) { return false; }
			};

		if (validateHmac("SHA-256", keyDataPtr, keyLen)) {
			keyData.name = "HMAC";
			keyData.hash = "SHA-256";
			keyData.isValid = true;
		}
		else if (validateHmac("SHA-1", keyDataPtr, keyLen)) {
			keyData.name = "HMAC";
			keyData.hash = "SHA-1";
			keyData.isValid = true;
		}
		else if (validateHmac("SHA-512", keyDataPtr, keyLen)) {
			keyData.name = "HMAC";
			keyData.hash = "SHA-512";
			keyData.isValid = true;
		}
		else if (validateHmac("MD5", keyDataPtr, keyLen)) {
			keyData.name = "HMAC";
			keyData.hash = "MD5";
			keyData.isValid = true;
		}
		else if (validateHmac("SHA3-256", keyDataPtr, keyLen)) {
			keyData.name = "HMAC";
			keyData.hash = "SHA3-256";
			keyData.isValid = true;
		}
	...
	public:
		JavaScriptMethod(JavaScript* InInstance, JSRuntime* InjsRuntime, JSContext* InjsContext) {
			...
			AppendMethod(ctx, global, "await", await);
		...
		static JSValue await(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			if (argumentCount != 1) {
		...
		static JSValue crypto_subtle_importKey(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			if (!ReadObjectProperty(ctx, js_algorithm, "name", js_a_name) || !ReadJSValueAsString(ctx, js_a_name, a_name)) {
				if (!js_algorithm.isValid()) promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.importKey] The algorithm must have the 'name' property"));
				else if (!js_algorithm.isValid()) promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.importKey] The algorithm name must be a valid string"));
				return promise.promise.get(1);
			}

			if (!allowedImportAlgorithm[format].count(a_name) || !allowedKeyUsagesList.count(a_name)) {
				promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.importKey] The algorithm name is not supported"));
				return promise.promise.get(1);
			}
			...
			//std::thread([=]() {
				...
				if (format != "jwk") {
					if (!ReadJSValueAsArrayBufferView(ctx, js_keyData, keyBinary)) {
						promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.importKey] The key data must be a ArrayBuffer when the format is 'raw'"));
						return promise.promise.get(1);
					}
				}
				else {
				...
				PKDATA pkd = GetPKData(&keyBinary);
				KEYDATA kd = GetKeyData(&keyBinary);
				if (pkd.isValid) {

					if (format == "raw" || (pkd.isPrivate && format == "spki") || (!pkd.isPrivate && format == "pkcs8")) {
						promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.importKey] Cannot import the '" + pkd.name + "' key as '" + format + "' format"));
						return promise.promise.get(1);
					}

					if (pkd.name == "RSA" && a_name.find("RSA") != std::string::npos) {
					...
					else if (pkd.name == a_name) {

					}
					else {
						promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.importKey] The key algorithm is invalid"));
						return promise.promise.get(1);
					}
					...
					promise.Reject(ctx, NewTypeError(ctx, "[crypto.subtle.importKey] The key is invalid"));
					return promise.promise.get(1);
				}
		...
		static JSValue crypto_subtle_generateKey(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			if (a_name == "HMAC") {
				...
				if (hash_name != "SHA-1" && hash_name != "SHA-256" && hash_name != "SHA-384" && hash_name != "SHA-512") {
				...
				if (a_hash_name != "SHA-1" && a_hash_name != "SHA-256" && a_hash_name != "SHA-384" && a_hash_name != "SHA-512") {
		...
		static JSValue console_log(JSContext* ctx, JSValueConst thisVal, int argumentCount, JSValueConst* argumentValues) {
			...
			auto get_color_value = [&](const std::wstring& color_key) -> const std::wstring& {
				...
				static std::wstring default_color = TextLightColorValue[L"Object"];
			...
			std::function<void(JSValueConst, int)> print_js_value;
			print_js_value = [&](JSValueConst val, int indent) -> void {
				...
				if (JS_IsFunction(ctx, val)) {
					...
					CreateOutput(func_str, get_color_value(L"Function"));
					return;
				}
				...
				if (JS_IsObject(val)) {
					...
					if (tag_str == "Promise") {
						...
						CreateOutput(promise_str, get_color_value(L"Object"));
						return;
					}
					...
					if (tag_str == "Date") {
						...
						CreateOutput(date_str, get_color_value(L"Date"));
						return;
					}
					...
					if (tag_str == "RegExp") {
						...
						CreateOutput(regex_str, get_color_value(L"RegExp"));
						return;
					}
					...
					if (tag_str == "Array") {
						...
						CreateOutput(indent_str + L"Array [", get_color_value(L"Array"));

						for (uint32_t i = 0; i < arr_length; ++i) {
							JSValue elem_val = JS_GetPropertyUint32(ctx, val, i);
							if (i > 0)
								CreateOutput(L", ", get_color_value(L"Array"));
					...
					if (tag_str == "Uint8Array" || tag_str == "Uint16Array" || tag_str == "Uint32Array" ||
						...
						JS_ToUint32(ctx, &byteLength, byteLength_val);
						JS_FreeValue(ctx, byteLength_val);

						CreateOutput(indent_str + stringToWstring(tag_str) + L" [", get_color_value(L"Array"));

						for (uint32_t i = 0; i < byteLength; ++i) {
							JSValue idx_val = JS_GetPropertyUint32(ctx, val, i);
							if (i > 0)
								CreateOutput(L", ", get_color_value(L"Array"));
							if (!is_private_value(idx_val) && !JS_IsException(idx_val)) {
								print_js_value(idx_val, indent + 1);
							}
							else if (is_private_value(idx_val)) {
							}
							else {
								CreateOutput(L"[invalid]", get_color_value(L"Comment"));
							}
				...
				if (JS_IsString(val)) {
					...
					if (!obj_type_name.empty() && obj_type_name != L"Object") {
						CreateOutput(obj_type_name + L" ", get_color_value(L"Type"));
					}
					...
					for (size_t i = 0; i < final_prop_size; i++) {
						...
						if (prop_name && *prop_name != '\0') {
							...
							CreateOutput(stringToWstring(prop_name) + L": ", get_color_value(L"Object"));
		...
		static bool AppendMethod(JSContext* ctx, JSV targetObject, std::string name, JSV func, int64_t flags = -1) {
			...
			if (flags != -1 && flags >= 0) {
				return JS_DefinePropertyValueStr(ctx, targetObject.get(0), name.c_str(), func.get(1), static_cast<unsigned int>(flags)) == 0;
			}
			return JS_SetPropertyStr(ctx, targetObject.get(0), name.c_str(), func.get(1)) == 0;
		}
		...
		static bool SetAttribute(JSContext* ctx, JSV targetObject, std::string key, JSV value, int64_t flags = -1) {
			...
			int ret = 0;
			if (flags != -1 && flags >= 0) {
				return (JS_DefinePropertyValueStr(ctx, targetObject.get(0), key.c_str(), value.get(1), static_cast<unsigned int>(flags))) == 0;
			}
			return (JS_SetPropertyStr(ctx, targetObject.get(0), key.c_str(), value.get(1))) == 0;
		}
		...
		static bool SetAttribute(JSContext* ctx, JSV targetObject, JSV key, JSV value, int64_t flags = -1) {
			...
			int ret = 0;
			if (flags != -1 && flags >= 0) {
				return (JS_DefinePropertyValue(ctx, targetObject.get(0), JS_ValueToAtom(ctx, key.get(0)), value.get(1), static_cast<unsigned int>(flags))) == 0;
			}
			return (JS_SetProperty(ctx, targetObject.get(0), JS_ValueToAtom(ctx, key.get(0)), value.get(1))) == 0;
		}
		static bool RemoveAttribute(JSContext* ctx, JSV targetObject, std::string key) {
			...
			bool ret = JS_DeleteProperty(ctx, targetObject.get(0), atom, 0) == 0;
		...
		static bool ReadJSValueAsArray(JSContext* ctx, JSV jsVal, std::vector<JSV>& outArray) {
			...
			for (uint32_t i = 0; i < arrLen; ++i) {
				...
				outArray.push_back(JSV(ctx, &elem));
			}
		...
		static bool ReadJSValueAsObject(JSContext* ctx, JSV jsVal, OBJECT& outObject) {
					...
					if (JS_IsNull(propVal)) {
						...
						if (JS_ToBigInt64(ctx, &bigIntVal, propVal) == 0) {
							...
							if (JS_ToBigUint64(ctx, &bigUintVal, propVal) == 0) {
								...
								throw std::runtime_error("Failed to convert bigint");
							}
						}
					}
						...
						OBJECT nestedObj;
						JSV nestedJSV = JSV(ctx, &propVal);
						if (!ReadJSValueAsObject(ctx, nestedJSV, nestedObj)) {
							throw std::runtime_error("Failed to read nested object");
						}
			...
			if (propTab != nullptr) {
				uint32_t releaseStart = bSuccess ? propCount : (bSuccess ? propCount : (uint32_t)0);
				for (uint32_t i = releaseStart; i < propCount; ++i) {
	...
	private:
		...
		static bool crypto_subtle_generateKey_core(std::string algo, uint64_t length, BYTEBUFFER_PTR outBinary, ...)
		{
			...
			while (true) {
				...
				else if (key == "publicExponent") publicExponent = std::stoull(value);
			...
			if (algo.find("AES") != std::string::npos) {
				...
				if (curve == "P-256") ecParams.Initialize(CryptoPP::ASN1::secp256r1());
				...
				if (algo == "ECDSA") {
					if (hashName == "SHA-1") {
						CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA1>::PrivateKey eccPrivKey;
						CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA1>::PublicKey eccPubKey;
						...
						CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA384>::PrivateKey eccPrivKey;
						CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA384>::PublicKey eccPubKey;
						eccPrivKey.Initialize(rng, ecParams);
						eccPrivKey.MakePublicKey(eccPubKey);
						...
						CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA512>::PrivateKey eccPrivKey;
						CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA512>::PublicKey eccPubKey;
						eccPrivKey.Initialize(rng, ecParams);
						eccPrivKey.MakePublicKey(eccPubKey);
	...
	std::string GetRequestHeader(FCGX_Request* request) {
		...
		// 1. 严格的空指针校验，从源头避免非法访问
		if (request == nullptr || request->envp == nullptr) {
			return completeHeaders;
		}

		// 2. 用索引遍历替代指针自增，彻底杜绝死循环（更安全的遍历方式）
		char** envp = request->envp;
		for (int i = 0; envp[i] != nullptr; ++i) { // 明确终止条件：遇到nullptr停止
			std::string envStr = envp[i]; // 直接通过索引访问，避免指针越界
